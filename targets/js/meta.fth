( Metacompiler for JavaScript target. )
...
defer ?colon
' noop is ?colon
: .colon   ." var colon_runtime = " here .a ." ;" cr  ['] noop is ?colon ;
: colon?   2dup s" :" compare 0= if ['] .colon is ?colon then ;
...
: forward: ( "name" -- )   create-forward  does> forward, ;
...
: .code1   ." function " latestxt >name .mangled
   ." _code (IP, xt)" cr s"     return IP;" latestxt ;

: .code2   ." function " source >in @ /string type cr s" "
   parse-name save-function-name ;
...
: warm?   s" warm" compare 0= ;
: .warm   ." var warm_word = " here .a ." ;" cr ;
: ?warm   2dup warm? if .warm then ;

: ]   compiler-context  ['] number, is number ;
: constant   create , does> @ ;
: variable   create cell allot ;
: :   parse-name colon? ?warm header, postcode dodoes  ] ( !csp )  does> >r ;
...
: cell   1 t-postpone literal ; immediate
: chars ; immediate
...
: does>   t-postpone (does>) ?colon ; immediate
...
: .a ( xt a1 -- xt )   over .ref over - case
      ...
      dup ." [8].slice(" body-offset - cell / (.) ." )"
   endcase ; *)
: .ref   ?dup if dup .a ." /*" >name type ." */" else ." 0" then ;
: .t-addr
...
: .,   ." , " ;
: .string   15 bounds do i c@ (.) ., loop ;
: .name   ." /*" dup .a space >name 2dup type ." */ " (.) .,  .string ;
: .link   >nextxt .ref ., ;
...
: .word   2 spaces  dup .name  dup .link  dup .does  dup .code  .body cr ;
